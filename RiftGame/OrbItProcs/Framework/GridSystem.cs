        ...
        for (int y = 0; y < cellsY; y++) {
          ...
          foreach (float dist in distToOffsets.Keys) {
            ...
            foreach (Tuple<int, int> tuple in distToOffsets[dist]) {
              if (tuple.Item1 + x < 0 || tuple.Item1 + x >= cellsX || tuple.Item2 + y < 0 || tuple.Item2 + y >= cellsY)
                continue;
              if (!offsetsArrayCollection[x, y].ContainsKey(dist))
                offsetsArrayCollection[x, y][dist] = new List<Tuple<int, int>>();
