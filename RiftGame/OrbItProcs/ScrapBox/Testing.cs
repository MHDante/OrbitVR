...
namespace OrbItProcs {
  public class Testing {
    ...
    public static void PrintTimer(string timer, bool reset = true) {
      if (usingMod && count%modcount != 0) return;
      if (!stopwatches.ContainsKey(timer)) {
        /*Console.WriteLine(timer + " doesn't have a stopwatch.");*/
        return;
      }
      Console.WriteLine(timer + "\t:\t" + stopwatches[timer].Elapsed + "\t" + stopwatches[timer].ElapsedMilliseconds +
                        "\t" + stopwatches[timer].ElapsedTicks);
    ...
    public void OldTests() {
      ...
      Action<int, float, string> del2 =
        (Action<int, float, string>) Delegate.CreateDelegate(typeof (Action<int, float, string>), room, testmethod);
      ...
      Func<Component, bool> delGet =
        (Func<Component, bool>) Delegate.CreateDelegate(typeof (Func<Component, bool>), minfo);
      ...
      Action<Component, bool> delSet =
        (Action<Component, bool>) Delegate.CreateDelegate(typeof (Action<Component, bool>), minfo);
      delSet(tester, false);
